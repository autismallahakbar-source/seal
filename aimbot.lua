...
-- Aimbot (E) + Spin + Triggerbot + ESP (G) + Fly + Noclip + Speed + Checks + Seal's Photo + Stealth (=)
-- =============================================
...
local Seal's photoTab = Window:CreateTab("Seal", nil)
...
local function addToWhiteList(name)
    ...
    WhiteList[name] = true
    -- Синхронизируем с реальным именем
...
local function isTeammate(player)
    ...
    for _, attr in ipairs(attrNames) do
        ...
        if myVal ~= nil and theirVal ~= nil and myVal == theirVal then
            return true
        end
    end

    if _G.Teams and type(_G.Teams) == "table" then
        local myTeam = _G.Teams[LocalPlayer.UserId] or _G.Teams[LocalPlayer.Name]
        local theirTeam = _G.Teams[player.UserId] or _G.Teams[player.Name]
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    ...
    if input.KeyCode == Enum.KeyCode.E then
        ...
        Rayfield:Notify({
            Title = "Aimbot",
            Content = Settings.AimbotEnabled and "Enabled" or "Disabled",
            Duration = 2,
        })
    end
    ...
    if input.KeyCode == Enum.KeyCode.G then
        ...
        Rayfield:Notify({
            Title = "ESP",
            Content = Settings.ESPEnabled and "Enabled" or "Disabled",
            Duration = 2,
        })
    end
    ...
    if input.KeyCode == Enum.KeyCode.Equals then
        ...
        Rayfield:Notify({
            Title = "Stealth Mode",
            Content = Settings.StealthMode and "ON (visuals hidden)" or "OFF",
            Duration = 2,
        })
    end
end)
...
Rayfield:Notify({
    ...
    Content = "E = Aimbot | G = ESP | = = Stealth",
    Duration = 4,
})
